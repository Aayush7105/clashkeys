...
export default function MultiplayerLeaderboard({
  ...
  if (rows.length === 0) {
    return <div className="text-sm text-[#646669]">No players found in this room.</div>;
  }
    ...
    <div className="space-y-2">
      {rows.map((row, index) => {
        ...
        return (
          <div
            key={row.id}
            className={`grid grid-cols-[2.5rem_minmax(0,1fr)_4rem_4.5rem_3.5rem] items-center gap-2 rounded-lg border px-3 py-2 text-sm ${
              isCurrentUser
          ...
          >
            ...
            <div className="text-right text-[#d1d0c5]">{Math.round(row.wpm)}</div>
      ...
      <div className="grid grid-cols-[2.5rem_minmax(0,1fr)_4rem_4.5rem_3.5rem] gap-2 px-3 pt-1 text-[10px] uppercase tracking-[0.2em] text-[#646669]">
        <div>rank</div>
        <div>player</div>
